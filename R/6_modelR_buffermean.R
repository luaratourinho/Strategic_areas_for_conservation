...
target_species <- read.csv("./04_n_thinned_records_byme.csv",
...
clean_df <- read.csv("./04_clean_byme.csv",
                     stringsAsFactors = FALSE, sep=";") %>%
...
target_species <- target_species[c(15,19)]
...
wc <- list.files("./extent/pca_soil_newscript",
                 pattern = "tif$",
                 full.names = TRUE) %>%
  stack()
...
foreach(
  ...
  partition_type <- ifelse(nrow(species_df) > 20, "crossvalidation", "bootstrap")
