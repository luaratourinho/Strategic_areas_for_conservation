...
n <- length(results_per_sp@data[,2:29])
g <- list()
results_per_sp_sf <- st_as_sf(results_per_sp)

for (i in 1:n) {
  g[[i]] <-
   ggplot(results_per_sp_sf) +
    geom_sf(aes_string(fill = colnames(results_per_sp@data[,2:29])[i])) + 
    theme_bw() +
    theme(
...
ggsave(
  p_arrange,
  file = "./Figures/Final_results/Conser_1_8.tiff",
  height = 50,
  width = 30,
  units = "cm"
)
...
ggsave(
  p_arrange2,
  file = "./Figures/Final_results/Conser_9_16.tiff",
  height = 50,
  width = 30,
  units = "cm"
)
...
ggsave(
  p_arrange3,
  file = "./Figures/Final_results/Conser_17_23.tiff",
  height = 50,
  width = 30,
  units = "cm"
)
